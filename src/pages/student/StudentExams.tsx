    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-background">
        <div className="text-center space-y-4">
          ...
          <p className="text-foreground text-lg font-medium">Carregando provas...</p>
        </div>
      </div>
    );
  }
      ...
      <header className="bg-card border-b border-border sticky top-0 z-40">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-display font-bold text-foreground">Minhas Provas</h1>
              <p className="text-sm text-muted-foreground">
                {passedExams.length} de {exams.length} provas aprovadas
              </p>
            </div>
          </div>
        </div>
          ...
          <Card className="border-dashed border-2 border-muted">
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-sm text-muted-foreground text-center max-w-sm">
                Complete as aulas dos módulos para desbloquear as provas
              </p>
            </CardContent>
          </Card>
        ) : (
                ...
                {pendingExams.length === 0 ? (
                  <Card>
                    <CardContent className="py-8 text-center">
                      ...
                      <p className="text-foreground font-semibold">Todas as provas foram realizadas!</p>
                    </CardContent>
                  </Card>
                ) : (
                ...
                {completedExams.length === 0 ? (
                  <Card>
                    <CardContent className="py-8 text-center">
                      ...
                      <p className="text-foreground font-semibold">Nenhuma prova realizada ainda</p>
                    </CardContent>
                  </Card>
                ) : (
                            ...
                            <div>
                              <div className="flex items-center gap-2 mb-1">
                                ...
                                <Badge 
                                  ...
                                  className="text-xs"
                                >
                                  {exam.submission?.passed ? "Aprovado" : "Reprovado"}
                                </Badge>
                              </div>
                              <h3 className="font-semibold text-foreground">
                                {exam.title}
                              ...
                              <div className="flex items-center gap-4 mt-2 text-sm">
                                <span className="font-bold text-foreground">
                                  Nota: {exam.submission?.score}%
                                </span>
                                <span className="text-muted-foreground">
                                  Mínimo: {exam.passing_score}%
                                </span>
                              </div>
                            </div>
                          </div>
